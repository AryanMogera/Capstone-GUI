...
from core.config import DEFAULT_BAUD, DEMO_MODE
...
class MainWindow(QtWidgets.QWidget):
    ...
    def __init__(self) -> None:
        ...
        self._resetTimer = QtCore.QTimer(self)
        self._resetTimer.setSingleShot(True)
        self._resetTimer.timeout.connect(self._on_reset_timeout)
    ...
    def _on_reset_clicked(self) -> None:
        ...
        self.statusLab.setText(f"Sending reset: {STM_RESET_CMD} (waiting for ACK)")
        self._resetTimer.start(2000)
    ...
    def _on_reset_timeout(self) -> None:
        if not self._reset_pending:
            return
        self._reset_pending = False
        self.statusLab.setText("Reset sent (no ACK). If STM reset, reconnect.")
    ...
    def onPacket(self, pkt: Dict[str, Any]) -> None:
        # Firmware messages / reset ACK
        msg = pkt.get("msg", None)
        if isinstance(msg, str) and msg.strip():
            if msg.strip() == "STM resetting":
                self._reset_pending = False
                if self._resetTimer.isActive():
                    self._resetTimer.stop()
                self.statusLab.setText("STM resetting… (port may disconnect)")
            else:
                self.statusLab.setText(msg.strip())
        ...
        def _pack_label_from_i(i_val: float) -> str:
            mag = abs(i_val)
            if i_val > 1e-6:
                flow = "Out of pack"
            elif i_val < -1e-6:
                ...
                flow = ""
            return f"Pack I: {mag:.2f} A {flow}".strip() if flow else f"Pack I: {mag:.2f} A"
        ...
        if pack_i is not None:
            try:
                ...
                pack_part = _pack_label_from_i(show_i)
            except Exception:
                pass

        if pack_part is None and self._pack_i_display_smooth is not None:
            pack_part = _pack_label_from_i(self._pack_i_display_smooth)
